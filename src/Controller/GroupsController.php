...
class GroupsController extends AppController {
    ...
    public function index() {
        /* $user_level = $this->Auth->user('access_level');
          if ($user_level == 1) {
          $groups = $this->getAllGroups();
          } else if ($user_level == 'GGM') {
          $groups = $this->getOrganizationGroups($this->Auth->user('user_id'));
          } else if ($user_level == 'GM') {
          $groups = $this->getGroupsbyCreator($this->Auth->user('user_id'));
          }else {
          $groups = $this->getAllGroups();
          } */
        $user = $this->Auth->user();
        $groups = $this->Groups->GetGroups($user['Location'], $this->org_id);
    ...
    public function isAuthorized($user) {
        ...
        if (isset($user['access_level'])) {
            $accessList = $this->AccessRights($user['access_level']);

            if (in_array($method, $accessList, TRUE))
            {
                return true;
            }
